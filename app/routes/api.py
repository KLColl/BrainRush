...
# Декоратор для API аутентифікації
...
def get_item(item_id):
    """Отримати конкретний товар магазину"""
...
def api_create_feedback():
    ...
    if not data or "message" not in data:
        return jsonify({"error": "Повідомлення обов'язкове"}), 400
...
# -----------------------
# ENDPOINTS СТАТИСТИКИ ІГОР
# -----------------------
@api_bp.route("/stats/games", methods=["GET"])
@api_login_required
def get_user_games():
    """Отримати список ігор, в які грав користувач"""
...
def get_game_stats(game_name):
    """Отримати детальну статистику для конкретної гри"""
...
@api_login_required
def update_theme():
    """Оновити налаштування теми користувача"""
    data = request.get_json()
    ...
    if theme not in ["light", "dark"]:
        return jsonify({"error": "Невалідна тема"}), 400
